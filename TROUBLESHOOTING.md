...
- 플러그인 설정의 `빠른 시작 -> 기본 구조 만들기` 버튼 실행
- 플러그인 설정의 `빠른 시작 -> 시작 세트 설치` 버튼으로 starter kit이 들어갔는지 확인
- 경로 설정이 비어 있거나 오탈자인지 확인
- 뉴스 폴더가 커스텀 경로라면 `뉴스읽기 폴더` 값을 재확인

## 다른 사용자 계정으로 옮겼더니 경로가 이상할 때

증상:

- vault 안에 `Users/...` 같은 폴더가 생김
- 이전 사용자 홈 디렉토리 경로가 설정에 남아 있음

원인:

- `홈페이지 노트 경로`, `뉴스읽기 폴더`에 Finder 절대경로가 저장됐을 수 있음

해결:

1. `Settings -> homepage`를 엽니다.
2. `홈페이지 노트 경로`, `뉴스읽기 폴더`를 확인합니다.
3. 최신 버전은 절대경로를 현재 vault 기준 경로로 자동 정리합니다.
4. 그래도 이상하면 아래 기본값으로 직접 넣습니다.
   - `홈/홈페이지.md`
   - `3. 뉴스읽기`

## 홈페이지 템플릿이 최신 구조로 보이지 않을 때
...
3. 재생성 후 아래 구조 확인
   - 상단 카드가 `오늘의 공지 / 오늘의 출석 / 우리반 상점 / 우리반 리포트` 4개만 보이는지
   - 카드 아래에 `오늘 한 줄 요약` 자동 문단이 있는지
   - 홈페이지 노트 섹션이 `오늘의 공지 / 오늘의 출석 / 우리반 상점 / 우리반 리포트 / 교사용 학생 관계 그래프` 기준으로 정리됐는지
